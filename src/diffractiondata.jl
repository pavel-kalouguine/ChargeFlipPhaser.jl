...
# Returns:
- A new `DiffractionData` object with an empty vector of Bragg peaks and the specified symmetry group.
"""
...
element g∈G, the following condition must hold: `g.a'*r*g.a = r`. This function computes 
the matrix `r` and returns the tolerance to which this condition is satisfied.
...
# Description:
This function creates a new `BraggPeaksOrbit` object corresponding to the wave vector `k` and adds it to the
`DiffractionData` object `dd`. If the wave vector is already present in the diffraction data, it will not be added again. The function checks if the orbit is extinct (i.e., if it belongs to an extinct orbit) and throws an error if so.
